...
export function Form({ showLoading, handleSubmit }: FormProps) {
  ...
  function handleFormSubmit() {
    ...
    if (inputMethod === 0) {
      setFormError("Please select an input method.");
      return;
    }
    ...
    if (inputMethod === 1) {
      if (textAreaValue.length < 500) {
        setFormError(
          `Article must contain at least 500 characters. Current: ${textAreaValue.length}`,
        );
        return;
      }
      handleSubmit(`---Post: ${textAreaValue}---`);
    }

    if (inputMethod === 2) {
      if (!URL_REGEX.test(inputUrlValue)) {
        setFormError("Please enter a valid URL.");
        return;
      }
      handleSubmit(inputUrlValue);
    }
  }
      ...
      <div className="col-12">
        <div className="mb-3">
          ...
          <div className="form-check">
            <input
              checked={inputMethod === 1}
              onChange={() => setInputMethod(1)}
          ...
          <div className="form-check">
            <input
              checked={inputMethod === 2}
              onChange={() => setInputMethod(2)}
              className="form-check-input"
              type="radio"
              name="flexRadioDefault"
              id="articleUrl"
        ...
        <div className={"mb-3 " + inputMethod1}>
          ...
          <textarea
            className="form-control form-textarea mb-1"
            id="inputText"
            name="inputText"
            placeholder=""
          ...
          {inputMethod === 1 &&
            ...
            textAreaValue.length < 500 && (
              <small className="text-muted">
                {textAreaValue.length} / 500 characters minimum
              </small>
            )}
        </div>
        ...
        <div className="sp">
          <button
            ...
            id="submitBtn"
            className={
              showLoading ? "loading sparkle-button" : "sparkle-button"
            }
